...
svg.lucide { stroke:#52607A; stroke-width:1.55; filter:drop-shadow(.25px .35px 0 rgba(168,144,120,.20)); }

`;

const LIGHT_COLORS = {
  // 기본 테마: 남색 + 깨끗한 아이보리/블루그레이
  page: "#F5F7FB",
  paper: "#FFFFFF",
  card: "#FFFFFF",
  ruleLine: "#DDE3EE",
...
- 시간표의 월~일 모든 열을 끝까지 확인하고, 보이는 모든 수업을 빠짐없이 추출
- 한 과목이 여러 요일이면 반드시 각 요일을 별도 항목으로 모두 출력 (예: 화/목 수업이면 화 1개 + 목 1개)
- 시간은 24시간 HH:MM
...
- JSON 배열만 출력. 설명·코드블록 금지.`;
    ...
    // AI가 같은 과목의 여러 요일을 "화/목", "화, 목", ["화","목"]처럼
    // 한 항목에 묶어 보내도 요일별 수업으로 모두 펼칩니다.
    const arr = (Array.isArray(parsedRows) ? parsedRows : [])
      .flatMap((x) => {
        ...
        const base = {
          ...
          start: normTime(x.start),
          end: normTime(x.end),
        };
        const rawDays = Array.isArray(x.day) ? x.day.join("") : String(x.day || "");
        const days = [...new Set(rawDays.match(/[월화수목금토일]/g) || [])];
        return days.map((day) => ({ ...base, day }));
      })
      .filter((x) => x.name && x.start && x.end && timeToMin(x.end) > timeToMin(x.start));
        ...
        <div className="gingham" style={{ backgroundColor: COLORS.paper, borderRadius: 31, overflow: "hidden", position: "relative", minHeight: 760, display: "flex", flexDirection: "column", fontFamily: "'Gowun Dodum', 'IBM Plex Sans KR', sans-serif", color: COLORS.ink, boxShadow: "inset 0 0 0 1px rgba(255,255,255,.75)" }}>
          ...
          {activeTab === "home" && (
            ...
            {(() => {
              ...
              return (
                <>
                  <div className="mb-3" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,alignItems:"stretch"}}>
                    {/* 홈 요약 카드는 내용 길이와 무관하게 같은 폭/낮은 높이를 유지 */}
                    <div className="min-w-0 rounded-2xl p-3 overflow-hidden" style={{background:COLORS.paper,border:`1px solid ${COLORS.ruleLine}`,height:92}}>
                      <div className="text-[10px] mb-1" style={{color:COLORS.muted}}>오늘 봉룡학사</div>
                      {m ? (
                        <div className="text-[11px] leading-snug overflow-hidden" style={{maxHeight:52}}>
                    ...
                    <button onClick={() => setActiveTab("jobs")} className="min-w-0 rounded-2xl p-3 text-left overflow-hidden"
                      style={{background:soon.length?COLORS.strawberry:COLORS.paper,color:soon.length?"#fff":COLORS.ink,border:`1px solid ${COLORS.ruleLine}`,height:92}}>
            ...
            <div className="px-4 pt-4 pb-6">
              ...
              {jobs && (() => {
                ...
                const Card = ({ j }) => {
                  ...
                  return (
                    <div className="rounded-xl px-2.5 py-2 mb-1.5" style={{background:COLORS.card,border:`1px solid ${urgent?COLORS.strawberry:COLORS.ruleLine}`}}>
                      <div className="flex items-start gap-2">
                        <div className="flex-1">
                          <a href={j.url} target="_blank" rel="noreferrer" className="text-[11px] font-semibold leading-snug line-clamp-2" style={{color:COLORS.ink}}>{j.title}</a>
                      ...
                      <div className="flex gap-1 mt-1.5">
                        <button onClick={() => jobToTask(j)} className="text-[10px] px-2 py-0.5 rounded-full flex-1"
                          style={{background:COLORS.paper,border:`1px solid ${COLORS.ruleLine}`}}>할 일에 담기</button>
                        <button onClick={() => hideJob(j.id)} className="text-[10px] px-2 py-0.5 rounded-full"
                            ...
                            <div className="flex gap-1.5 mb-2">
                              {UNDERSTAND.map((u) => (
                                <button key={u.key} onClick={() => patchEntry(openBook, en.id, { understand: en.understand === u.key ? null : u.key })}
                                  className="text-[10px] px-2 py-0.5 rounded-full flex-1"
